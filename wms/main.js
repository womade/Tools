...
if (navigator.userAgent.toLowerCase().indexOf('micromessenger') !== -1) loadScript('https://t.ssss.fun/wms/FuckWeChat.js');

if (!window.Promise) loadScript('https://t.ssss.fun/wms/polyfill.min.js');

var avatarURL = [];
xhrGet('https://t.ssss.fun/wms/avatarURL.json', function (result) { avatarURL = JSON.parse(result) });
var emoticon = [];
xhrGet('https://t.ssss.fun/wms/emoticon.json', function (result) { emoticon = JSON.parse(result) });
...
document.getElementById('avatar').style.backgroundImage = 'url(' + (localStorage.getItem('avatar') || 'https://t.ssss.fun/wms/avatar.png') + ')';
...
function getArticleInfo() {
    ...
    mdui.prompt('公众号文章链接', function (value) {
        var xhr = new XMLHttpRequest;
        xhr.open('GET', 'https://wms.made.workers.dev/?url=' + encodeURIComponent(value)); //获取标题和封面的服务器
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                var result = JSON.parse(xhr.responseText);
                document.getElementById('requestAction').innerText = '确定';
        ...
        document.getElementById('requestAction').innerText = '取消';
...
document.getElementById('generate').addEventListener('click', function () {
    ...
    html2canvas(document.getElementById('fakeWechatMoment'), {
        ...
        mdui.alert(''
            ...
            +     '<p>错误信息：</p>'
            +     '<pre>' + error + '</pre>'
            +     '<p>你可以截图发送邮件至 admin@ssss.fun 向作者反馈哦~</p>'
            + '</div>',
            '生成失败'
        );
    }).finally(function () {
